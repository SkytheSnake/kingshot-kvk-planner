...
function showAdminApp(show){
  ...
  if(show){
    ...
    const isOwner=admin.admin_role==="owner";
    if($("ownerToolsTabButton")) $("ownerToolsTabButton").hidden=!isOwner;
    if($("resetKvkBtn")) $("resetKvkBtn").hidden=!isOwner;
    if($("archiveCycleBtn")) $("archiveCycleBtn").hidden=!isOwner;
    if($("adminPasswordManager")) $("adminPasswordManager").hidden=!isOwner;
    setAdminSection("planner");
  }else{
    if($("resetKvkBtn")) $("resetKvkBtn").hidden=true;
    if($("archiveCycleBtn")) $("archiveCycleBtn").hidden=true;
    if($("adminPasswordManager")) $("adminPasswordManager").hidden=true;
    if($("ownerToolsTabButton")) $("ownerToolsTabButton").hidden=true;
    setAdminSection("planner");
  }
}
